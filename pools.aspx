...
<%
	...
	if submit = "Login" then
		...
		if res.toupper() = username.toupper() then
			...
			session("page_message") = "Invalid username/password, please try again."
			response.redirect("error.aspx", true)
...
<div id="Content">
		<form class="cmxform">
			...
			<fieldset>
				<legend>Login</legend>
				<ol>
					<li><label>Username: </label><input type="text" name="username" id="username" /></li>
					<li><label>Password: </label><input type="password" name="password" id="password" /></li>
					<li><label>Remember Login? </label><input type="checkbox" name="rememberlogin" /></li>
